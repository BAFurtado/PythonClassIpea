...
def main(database, path, query_params):
    ...
    print(database.sort_values(by='id', ascending=False))
    return database


if __name__ == '__main__':
    interrupt = False
    # for each in ['OGlobo_Rio', 'valoreconomico', 'folha', 'Estadao', 'correio', 'opovo', 'gzhdigital', 'em_com',
    #              'Poder360']:
    for each in ['furtadobb', 'phgfsouza', 'RafaelGOsorio', 'RaphaelGouvea_',
                 'rodfracalossi', 'AG_Pineli']:
        if interrupt:
            break
        if len(sys.argv) == 2:
            each = sys.argv[1]
            interrupt = True
        query = {'query': f'(from:{each})',
                 'tweet.fields': 'author_id'}
        p = '/home/furtado/Documents/Professor/Python2022/data'
        f = f'{each}.csv'
